...
#include "scanner_settings.hpp" // Если необходимо для load_settings_from_txt
...
// Если ScannerSettingsManager определен в другом заголовочном файле (например scanner_settings.hpp)
// и мы его используем:
...
bool EnhancedSettingsManager::save_settings_to_txt(const DroneAnalyzerSettings& settings) {
    ...
    auto header_result = settings_file.write(header.data(), header.size());
    ...
    auto content_result = settings_file.write(content.data(), content.size());
...
void EnhancedSettingsManager::remove_backup_file(const std::string& filepath) {
    (void)filepath; // Пока не реализовано в API
}
...
DroneAnalyzerSettingsManager& DroneAnalyzerSettingsManager::get_settings_manager() {
...
void DronePresetSelector::show_preset_menu(NavigationView& nav, PresetMenuView callback) {
    ...
    public:
        PresetMenuViewImpl(NavigationView& nav, std::vector<std::string> names, std::function<void(const DronePreset&)> on_selected,
    ...
    private:
        ...
        bool on_key(const KeyEvent key) override {
            if (key == KeyEvent::Select) {
                ...
                if (idx < presets_.size() && on_selected_fn_) {
    ...
    nav.push<PresetMenuViewImpl>(preset_names, callback, all_presets);
}
...
void DronePresetSelector::show_type_filtered_presets(NavigationView& nav, DroneType type) {
    ...
    auto on_selected = [&nav](const DronePreset& preset) { (void)preset; nav.pop(); };
...
bool DroneFrequencyEntry::is_valid() const {
    return SimpleDroneValidation::validate_frequency_range(frequency_hz) &&
           rssi_threshold_db >= -120 && rssi_threshold_db <= 0 && bandwidth_hz > 0;
}
...
void HardwareSettingsView::save_current_settings() {
    ...
    switch (field_spectrum_mode_.selected_index()) {
...
// AudioSettingsView
AudioSettingsView::AudioSettingsView(NavigationView& nav) : View(), nav_(nav), audio_settings_() {
...
DroneAnalyzerSettingsView::DroneAnalyzerSettingsView(NavigationView& nav) : View(), nav_(nav), current_settings_() {
    add_children({&text_title_, &button_audio_settings_, &button_hardware_settings_,
                  &button_scanning_settings_, &button_advanced_settings_, &button_load_defaults_,
                  &button_about_author_, &button_manage_db_});
...
DroneDatabaseListView::DroneDatabaseListView(NavigationView& nav)
    : View(), nav_(nav), entries_(DroneDatabaseManager::load_database()) 
{
...
void DroneDatabaseListView::on_entry_selected(size_t index) {
    if (index == 0) {
        nav_.push<DroneEntryEditorView>(DroneDbEntry{0, ""}, [this](const DroneDbEntry& entry) {
            if (entry.freq != 0) {
