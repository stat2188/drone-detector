 ...
 *
 * CRITICAL FIX #003: Enhanced lock order validation
 *
 * PROBLEM:
 * - Original implementation allowed acquiring two locks at same level in any order
 * - This does NOT prevent deadlock
 * - Example: Thread A acquires thread_mutex_ then state_mutex_
 *          Thread B acquires state_mutex_ then thread_mutex_
 *          Result: DEADLOCK!
 ...
 * - Deadlock is impossible if locks are acquired in ascending order
 *
 * This helper ensures locks are acquired in correct order
 * to prevent deadlocks. Use it when you have multiple locks.
 *
 * P1-HIGH FIX #4: Enhanced with compile-time lock order validation
 * - Validates lock order at compile time using static_assert
 * - Prevents lock order violations before code runs
 * - Integrates with MutexLock class for automatic validation
 *
 * USAGE:
 * @code
 *     void my_function() {
 *         LockOrderValidator validator(LockOrder::THREAD_MUTEX);
 *         MutexLock lock1(thread_mutex_, LockOrder::THREAD_MUTEX);
 *
 *         // Can acquire higher-order locks
 *         MutexLock lock2(state_mutex_, LockOrder::STATE_MUTEX);  // OK: STATE_MUTEX > THREAD_MUTEX
 *
 *         // Cannot acquire lower-order locks (would deadlock)
 *         // MutexLock lock3(data_mutex_, LockOrder::DATA_MUTEX);  // OK: DATA_MUTEX > STATE_MUTEX
 *     }
 * @endcode
 *
...
public:
     ...
     *
     * CRITICAL FIX #003: Constructor now stores current lock order
     * - Used to validate subsequent lock acquisitions
     * - Ensures locks are acquired in ascending order
     *
     * @note This is a no-op in release builds, but helps document intent
     */
    explicit constexpr LockOrderValidator(LockOrder order) noexcept : current_lock_(order) {
        (void)current_lock_;  // Suppress unused warning
    }

    /**
     * @brief Check if a higher-order lock can be acquired
     * @param next_lock The lock order to check
     * @return true if next_lock > current_lock (ascending order)
     *
     * CRITICAL FIX #003: Enforce ascending lock order
     * - Original implementation allowed acquiring locks at same level in any order
     * - This does NOT prevent deadlock
     * - Fixed to enforce next_lock > current_lock
     *
     * USAGE:
     * @code
     *     LockOrderValidator validator(LockOrder::THREAD_MUTEX);
     *     MutexLock lock1(thread_mutex_, LockOrder::THREAD_MUTEX);
     *
     *     // Check if we can acquire next lock
     *     if (validator.can_acquire(LockOrder::STATE_MUTEX)) {
     *         MutexLock lock2(state_mutex_, LockOrder::STATE_MUTEX);  // OK
     *     }
     *
     *     // This would fail (descending order)
     *     if (validator.can_acquire(LockOrder::DATA_MUTEX)) {
     *         // This would be allowed since DATA_MUTEX > STATE_MUTEX
     *     }
     * @endcode
     */
    constexpr bool can_acquire(LockOrder next_lock) const noexcept {
        return static_cast<uint8_t>(next_lock) > static_cast<uint8_t>(current_lock_);
    }

    /**
     * @brief Validate lock order at compile time (static_assert)
     * @tparam current_order The current lock order being held
     * @tparam next_order The lock order to validate against
     * @return true if next_order > current_order (for use in static_assert)
     *
     * P1-HIGH FIX #4: Compile-time lock order validation
     * Use this to validate lock order at compile time:
     * @code
     *     LockOrderValidator validator(LockOrder::THREAD_MUTEX);
     *     static_assert(validator.validate_order<LockOrder::THREAD_MUTEX, LockOrder::STATE_MUTEX>(),
     *                   "Invalid lock order: STATE_MUTEX must be acquired after THREAD_MUTEX");
     * @endcode
     *
     * CRITICAL FIX #003: Fixed to use template parameters
     * - Original implementation always compared against LockOrder::DATA_MUTEX
     * - This fix makes validation work correctly for any starting lock order
     */
    template <LockOrder current_order, LockOrder next_order>
    static constexpr bool validate_order() noexcept {
        return static_cast<uint8_t>(next_order) > static_cast<uint8_t>(current_order);
    }

private:
    LockOrder current_lock_;  ///< Current lock order being held
};
