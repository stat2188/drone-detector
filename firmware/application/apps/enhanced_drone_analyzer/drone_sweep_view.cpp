...
void DroneSweepView::save_settings() noexcept {
    ...
    // Read existing file content into shared static buffer
    size_t file_len = 0;
    ...
    // Write output: non-sweep lines from existing file, plus sweep keys (shared static buffer)
    size_t out_offset = 0;

    // If file didn't exist, write defaults for non-sweep settings
    if (file_len == 0) {
        out_offset += snprintf(g_settings_out_buf + out_offset, sizeof(g_settings_out_buf) - out_offset,
            ...
            if (i == file_len || g_settings_file_buf[i] == '\n' || g_settings_file_buf[i] == '\r') {
                if (i > line_start) {
                    ...
                    if (!is_sweep_line && (i - line_start) > 0) {
                        ...
                        if (out_offset + copy_len + 1 < sizeof(g_settings_out_buf)) {
                            __builtin_memcpy(g_settings_out_buf + out_offset, g_settings_file_buf + line_start, copy_len);
                            out_offset += copy_len;
                            g_settings_out_buf[out_offset++] = '\n';
                        }
                    }
                }
                // Skip \r\n pairs
    ...
    file.write(g_settings_out_buf, out_offset);
    (void)file.sync();
    file.close();
}
