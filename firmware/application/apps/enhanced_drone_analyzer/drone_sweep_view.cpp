...
void DroneSweepView::save_settings() noexcept {
    ...
    // Simple approach: rewrite the whole file with sweep keys updated
    File file;
    ...
    // Remove old sweep keys from buffer (mark with #)
    // Simplified: just write a fresh sweep section at the end
    // The load_startup_settings parser ignores lines starting with #

    // Overwrite or create file with all settings
    // We'll write sweep keys only — other settings remain from last save
    const auto create_result = file.create(settings_dir / u"eda_settings.txt");
    if (!create_result) return;

    // Write original content first (non-sweep lines)
