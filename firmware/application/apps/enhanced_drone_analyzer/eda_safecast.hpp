...
// SAFE NUMERIC CAST
// * * @brief Safe numeric cast with bounds checking * * Performs type-safe numeric conversion with compile-time and runtime bounds checking. * Triggers assertion failure in debug mode if value would overflow/underflow. * In release mode, clamps to target type range (graceful degradation). * * @tparam To Target type * @tparam From Source type * @tparam StrictMode If true, fails at compile time for potentially unsafe conversions (default: false) * * USAGE: * int64_t large_value = 1000000; * int32_t safe_value = SafeCast<int32_t, int64_t, false>::from(large_value); * * // For critical code paths requiring compile-time safety: * int32_t strict_value = SafeCast<int32_t, int64_t, true>::from(large_value); * * @note Compile-time error if types have incompatible signs and StrictMode is true * @note Runtime assertion in debug mode on overflow/underflow * @note Clamps to target range in release mode * @note In StrictMode, static_assert prevents compilation if conversion cannot be guaranteed safe
template<typename To, typename From, bool StrictMode = false>
class SafeCast {
public:
    // * * @brief Perform safe conversion from source to target type * @param value Value to convert * @return Converted value (clamped if out of range in release)
    ...
    // * * @brief Check if conversion is safe without performing it * @param value Value to check * @return true if conversion is safe, false otherwise
    ...
    // * * @brief Check if value would overflow when converted to target type * @param value Value to check * @return true if conversion would overflow
    ...
    // * * @brief Check if value would underflow when converted to target type * @param value Value to check * @return true if conversion would underflow
...
// * * @brief Safe reinterpret_cast with alignment checks * * Performs type-safe pointer reinterpretation with compile-time alignment validation. * Ensures target type alignment is compatible with source pointer. * * @tparam To Target pointer type * @tparam From Source pointer type * @param ptr Source pointer to cast * @return Casted pointer * * USAGE: * uint8_t buffer[1024]; * MyStruct* obj = safe_reinterpret_cast<MyStruct*>(buffer); * * @note Compile-time error if alignment is insufficient * @note Runtime assertion in debug mode if pointer is null
...
// SAFE BIT CAST
// * * @brief Safe bit-level cast between types * * Performs bit-level type conversion (like std::bit_cast in C++20). * Requires both types to have the same size and be trivially copyable. * Uses explicit byte-by-byte copy for C++14/17 constexpr compatibility. * * @tparam To Target type * @tparam From Source type * @param value Value to cast * @return Bit-casted value * * USAGE: * uint32_t int_value = 0x12345678; * float float_value = safe_bit_cast<float>(int_value); * * @note Compile-time error if types have different sizes * @note Compile-time error if types are not trivially copyable * @note Uses explicit byte copy for C++14/17 constexpr compatibility
template<typename To, typename From>
