...
void SpectrumPreviewWidget::paint(ui::Painter& painter) {
    ...
    for (int col = 0; col < w; col++) {
        ...
        int h_px;
        if (flat_top_px > 0 && dx <= flat_top_px) {
            h_px = peak_h;
        } else {
            h_px = v_shape_height(dx, half_width, peak_h, valley_margin_px);
        }
    ...
    // Elevated threshold line (dark grey solid)
    painter.draw_hline({x0, elevated_abs_y}, w, ui::Color::dark_grey());

    // Min/max width markers relative to peak center
    int min_w_px = static_cast<int>(min_width_) * w / 512;
    int max_w_px = static_cast<int>(max_width_) * w / 256;

    for (int side = -1; side <= 1; side += 2) {
        int ml = actual_center + side * min_w_px;
        if (ml >= 0 && ml < w) {
            painter.draw_vline({x0 + ml, elevated_abs_y - 2}, 5, ui::Color::cyan());
        }
        int mr = actual_center + side * max_w_px;
        if (mr >= 0 && mr < w) {
            painter.draw_vline({x0 + mr, elevated_abs_y - 2}, 5, ui::Color::grey());
        }
    }

    // Valley indicators (just outside signal half-width)
    for (int side = -1; side <= 1; side += 2) {
        int vx = actual_center + side * (half_width + 1);
