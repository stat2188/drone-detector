...
DroneSettingsView::DroneSettingsView(NavigationView& nav, const ScanConfig& config, DroneScanner* scanner_ptr, DroneDisplay* display) noexcept
    ...
    button_save_.on_select = [this](ui::Button&) {
        ...
        // Read existing file to preserve sweep keys (shared static buffer)
        size_t file_len = 0;
        ...
        if (!open_result) {
            ...
            while (true) {
                ...
                for (size_t i = 0; i < bytes && file_len < sizeof(g_settings_file_buf) - 1; ++i) {
                    g_settings_file_buf[file_len++] = static_cast<char>(chunk[i]);
                }
            }
        }
        file.close();
        g_settings_file_buf[file_len] = '\0';

        // Create/overwrite file for writing
        const auto create_result = file.create(settings_dir / u"eda_settings.txt");
        if (!create_result) {
            // Use shared static buffer for writing
            size_t offset = 0;

            // Write header
            offset += snprintf(g_settings_out_buf + offset, sizeof(g_settings_out_buf) - offset,
                "# Enhanced Drone Analyzer Settings\n");
            offset += snprintf(g_settings_out_buf + offset, sizeof(g_settings_out_buf) - offset,
                "# Auto-generated by EDA Settings\n\n");

            // Write non-sweep scanning settings
            offset += snprintf(g_settings_out_buf + offset, sizeof(g_settings_out_buf) - offset,
            ...
            for (size_t i = 0; i <= file_len; ++i) {
                if (i == file_len || g_settings_file_buf[i] == '\n' || g_settings_file_buf[i] == '\r') {
                    if (i > line_start) {
                        for (const auto& key : sweep_keys) {
                            const size_t klen = __builtin_strlen(key);
                            if ((i - line_start) >= klen &&
                                __builtin_memcmp(g_settings_file_buf + line_start, key, klen) == 0) {
                                const size_t copy_len = i - line_start;
                                if (offset + copy_len + 1 < sizeof(g_settings_out_buf)) {
                                    __builtin_memcpy(g_settings_out_buf + offset, g_settings_file_buf + line_start, copy_len);
                                    offset += copy_len;
                                    g_settings_out_buf[offset++] = '\n';
                                }
                                break;
                            }
                        }
                    }
                    if (i < file_len && g_settings_file_buf[i] == '\r' && (i + 1) < file_len && g_settings_file_buf[i + 1] == '\n') {
                        i++;
                    }
                    line_start = i + 1;
                }
            ...
            const auto write_result = file.write(g_settings_out_buf, offset);
...
void load_startup_settings(ScanConfig& config) noexcept {
    ...
    auto parse_line = [&config](const uint8_t* buf, size_t len) {
        ...
        if (key_matches("spectrum_detection")) {
            ...
            const int32_t sens = parse_int();
            config.rssi_threshold_dbm = -20 - (sens > 100 ? 100 : (sens < 0 ? 0 : sens));
        } else if (key_matches("enable_audio_alerts")) {
            // Loaded here for consistency; actual audio state set by DroneSettingsView
            (void)parse_bool();
        } else if (key_matches("show_spectrum")) {
            (void)parse_bool();
        } else if (key_matches("show_histogram")) {
