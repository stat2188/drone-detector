...
DroneSettingsView::DroneSettingsView(NavigationView& nav, const ScanConfig& config, DroneScanner* scanner_ptr, DroneDisplay* display) noexcept
    ...
    button_save_.on_select = [this](ui::Button&) {
        ...
        // Read existing file to preserve sweep keys
        ...
        if (!create_result) {
                    ...
                    if (i > line_start) {
                        for (const auto& key : sweep_keys) {
                            ...
                            if ((i - line_start) >= klen &&
                                ...
                                if (offset + copy_len + 1 < sizeof(out_buf)) {
                                    __builtin_memcpy(out_buf + offset, file_buf + line_start, copy_len);
                                    offset += copy_len;
                                    out_buf[offset++] = '\n';
                                }
                                break;
            ...
            const auto write_result = file.write(out_buf, offset);
