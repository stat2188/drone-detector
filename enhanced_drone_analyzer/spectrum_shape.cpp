...
SpectrumShape::AnalysisResult SpectrumShape::analyze(
    ...
    result.signal_left = sig_left;
    result.signal_right = sig_right;

    // Calculate avg_margin for sharpness check
    result.avg_margin = compute_avg_margin(spectrum, sig_left, sig_right, result.noise_floor);

    // ✅ Больше нет ранних возвратов. Все параметры вычисляются всегда.
    // ✅ Проверка фильтров теперь в отдельной функции pass_filters()

    result.signal_detected = pass_filters(spectrum, result, config);
    return result;
}
...
bool SpectrumShape::pass_filters(
    ...
    if (!check_sharpness(result.peak_margin, result.avg_margin, config.peak_sharpness)) return false;
    if (!check_peak_ratio(result.peak_margin, result.signal_width, config.peak_ratio)) return false;
    if (!check_valley_depth(spectrum, result.signal_left, result.signal_right, result.noise_floor, config.valley_depth)) return false;
    if (!check_flatness(spectrum, result.peak_index, result.signal_left, result.signal_right, config.flatness)) return false;
    if (!check_symmetry(result.peak_index, result.signal_left, result.signal_right, result.signal_width, config.symmetry)) return false;
