        ...
        }}
{
    ...
    button_settings_.on_select = [this](ui::Button&) {
        ...
        // ✅ Stop scanner BEFORE opening dialog - prevent stack overflow
        const bool was_scanning = scanning_;
        if (scanning_) {
            scanner_thread_->set_scanning(false);
            (void)scanner_ptr_->stop_scanning();
        ...
        // Open dialog - this will block UI thread
        (void)nav_.push<DroneSettingsView>(config, scanner_ptr_, &drone_display_);
        
        // ✅ Resume scanning when dialog closes
        nav_.set_on_pop([this, was_scanning]() {
            if (was_scanning) {
                baseband::spectrum_streaming_start();
                (void)scanner_ptr_->start_scanning();
                scanner_thread_->set_scanning(true);
    ...
    button_swp_.on_select = [this](ui::Button&) {
        ...
        // Open dialog - this will block UI thread
        (void)nav_.push<DroneSweepView>(config, scanner_ptr_);
...
void DroneScannerUI::on_sweep_spectrum(const ChannelSpectrum& spectrum) noexcept {
    ...
    if (win.pixel_index < COMPOSITE_SIZE) {
        ...
        if (win.f_center < win.f_max) {
            ...
            win.f_center = (win.f_center + SWEEP_STEP_ALIGNMENT_HZ / 2) / SWEEP_STEP_ALIGNMENT_HZ * SWEEP_STEP_ALIGNMENT_HZ;
            retune_sweep_window(win, nullptr);
            return;
        }
...
void DroneScannerUI::SweepWindow::init(FreqHz start, FreqHz end, FreqHz step) noexcept {
    ...
    // pixel_freq calculation to correctly map pixels to exception check frequencies.
...
void DroneScannerUI::capture_pattern_frame(const ChannelSpectrum& spectrum) noexcept {
    ...
    // Normalize spectrum to 16-bin waveform (stack-local buffer)
...
void DroneScannerUI::finalize_pattern_capture() noexcept {
    ...
    if (scanner_ptr_ != nullptr) {
        ...
        // Interpolate 16-element averaged waveform to 256-bin synthetic spectrum (stack-local)
        uint8_t synthetic_spectrum_buf[FFT_BIN_COUNT];
        constexpr size_t BINS_PER_WAVEFORM_ELEM = FFT_BIN_COUNT / PATTERN_WAVEFORM_SIZE;
        memset(synthetic_spectrum_buf, 0, sizeof(synthetic_spectrum_buf));
        ...
        for (size_t i = 0; i < PATTERN_WAVEFORM_SIZE; ++i) {
            ...
            for (size_t j = 0; j < BINS_PER_WAVEFORM_ELEM; ++j) {
                ...
                if (bin_idx < FFT_BIN_COUNT) {
                    synthetic_spectrum_buf[bin_idx] = value;
                }
            }
        }

        // Analyze synthetic spectrum to extract features (stack-local sort buffer)
        uint8_t spectrum_shape_sort_buf[256];
        const SpectrumShape::AnalysisResult shape = SpectrumShape::analyze(
            synthetic_spectrum_buf,
            spectrum_shape_sort_buf,
            shape_config
        );
...
void DroneScannerUI::match_patterns_in_sweep(const ChannelSpectrum& spectrum, FreqHz freq) noexcept {
    ...
    // Analyze spectrum shape (stack-local sort buffer)
    uint8_t spectrum_shape_sort_buf[256];
    SpectrumShape::Config shape_config{
        cfg.spectrum_margin,
